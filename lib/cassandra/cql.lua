...
--]]
  ...
  -- Define initial bytes for CRC32 computation
  local initial_bytes = "\xFA\x2D\x55\xCA"
  
  -- Define crc24 function
  local function crc24(data)
    local crc = 0x875060
    local poly = 0x1974F0B
  
    for i = 1, #data do
      local byte = string.byte(data, i)
      crc = bit.bxor(crc, bit.lshift(byte, 16))
      for _ = 1, 8 do
        if bit.band(crc, 0x800000) ~= 0 then
  ...
  local function crc32(payload)
    local data = initial_bytes .. payload
    local crc = ngx.crc32_long(data)
    -- Adjust for signedness
    if crc >= 0x80000000 then
      crc = crc - 0x100000000  -- Convert to signed 32-bit integer
    end
    return crc
  end
  ...
  do
    ...
    -- Add write_24bits_le method to Buffer class
    function Buffer:write_24bits_le(val)
      self:write(char(
        band(val, 0xFF),
      ...
      -- Add write_int_le method to Buffer class if not already present
    function Buffer:write_int_le(val)
      self:write(char(
        band(val, 0xFF),
        band(rshift(val, 8), 0xFF),
        band(rshift(val, 16), 0xFF),
    ...
    function Buffer:read_24bits_le()
      local b1 = self:read_byte()
      local b2 = self:read_byte()
      local b3 = self:read_byte()
      return b1 + lshift(b2, 8) + lshift(b3, 16)
    end
  
    function Buffer:read_int_le()
      local b1 = self:read_byte()
      local b2 = self:read_byte()
      local b3 = self:read_byte()
      local b4 = self:read_byte()
      local val = b1 + lshift(b2, 8) + lshift(b3, 16) + lshift(b4, 24)
  ...
  do
    ...
    -- Complete modified build_frame function
    function request_mt:build_frame(version)
      -- Check if this is the initial handshake (STARTUP or OPTIONS message)
      local is_initial_handshake = (self.op_code == OP_CODES.STARTUP) or (self.op_code == OP_CODES.OPTIONS)
    
      -- Determine if we should use the legacy frame format
      local use_legacy_format = is_initial_handshake or (version < 5)
    
      if use_legacy_format then
        -- Use legacy frame format
        local header, body = Buffer.new(version), Buffer.new(version)
        self:build_body(body)  -- Build the message body
        local body_data = body:get()
        ...
        if version < 3 then
          header:write_byte(0)  -- Stream ID (byte) for versions < 3
        else
          header:write_short(0)  -- Stream ID (short) for versions >= 3
        end
        ...
        -- Step 1: Build the Message Body (Big-endian)
        local body = Buffer.new(version)
        self:build_body(body)  -- Build the message body
        local body_data = body:get()
        local body_length = #body_data
    
        -- Step 2: Build the Envelope Header with Correct Body Length (Big-endian)
        local envelope = Buffer.new(version)
        envelope:write_byte(version)  -- Version byte (big-endian)
        local flags = 0
        if self.opts and self.opts.tracing then
          flags = bor(flags, FRAME_FLAGS.TRACING)
        end
        envelope:write_byte(flags)  -- Flags byte
        envelope:write_short(0)  -- Stream ID (big-endian)
        envelope:write_byte(self.op_code)  -- Opcode
        envelope:write_int(body_length)  -- Body length
    ...
    local function build_args(body, args, opts)
      ...
      if body.version >= 5 then
        -- For protocol v5 and above, flags is a 4-byte integer
        body:write_int(flags)
      else
        -- For protocol versions below 5, flags is a 1-byte value
    ...
    local req_mts = {
      ...
      prepare = {
        ...
        build_body = function(self, body)
          body:write_long_string(self.query)
          if body.version >= 5 then
            body:write_int(0) -- flags field, set to 0 if no flags are used
          end
        end
      },
      execute_prepared = {
        new = function(prepared_stmt, args, opts, query)
          local r = new_request(OP_CODES.EXECUTE)
          r.query_id = prepared_stmt.query_id
          r.result_metadata_id = prepared_stmt.result_metadata_id
          r.args = args
          r.opts = opts
        ...
        build_body = function(self, body)
          local flags = self.opts.flags or 0x00
          body:write_short_bytes(self.query_id)
          if body.version >= 5 and self.result_metadata_id then
            -- Include result_metadata_id
            body:write_short_bytes(self.result_metadata_id)
            flags = bor(flags, QUERY_FLAGS.METADATA_CHANGED)
          end
        ...
        build_body = function(self, body)
          ...
          for i = 1, #queries do
            ...
            if opts.prepared then
              ...
              body:write_byte(1)
              body:write_short_bytes(prepared_stmt.query_id)
            else
              body:write_byte(0)
